...
class GlocalTextPathCMTPreTraining(BertPreTrainedModel):
    ...
    def forward_mrc(
        ...
        view_idx = [len(x)-1 for x in traj_vpids]
        vp_view_lens = [x[view_idx[i]] for i, x in enumerate(torch.split(traj_vp_view_lens, traj_step_lens))]
        ...
        if traj_obj_img_fts is not None:
            vp_obj_lens = [x[view_idx[i]] for i, x in enumerate(torch.split(traj_vp_obj_lens, traj_step_lens))]
