...
class LocalVPEncoder(nn.Module):
    ...
    def vp_input_embedding(self, split_traj_embeds, split_traj_vp_lens, vp_pos_fts, traj_vpids):
        last_vp_idx = [len(x) for x in traj_vpids]
        vp_img_embeds = pad_tensors_wgrad([x[last_vp_idx[i]-1] for i, x in enumerate(split_traj_embeds)])
        vp_lens = torch.stack([x[last_vp_idx[i]-1]+1 for i, x in enumerate(split_traj_vp_lens)], 0)
    ...
    def forward(
        self, txt_embeds, txt_masks, split_traj_embeds, split_traj_vp_lens, vp_pos_fts, traj_vpids
    ):
        vp_embeds, vp_masks = self.vp_input_embedding(
            split_traj_embeds, split_traj_vp_lens, vp_pos_fts, traj_vpids
        )
...
class GlobalMapEncoder(nn.Module):
    ...
    def _aggregate_gmap_features(
        self, split_traj_embeds, split_traj_vp_lens, gmap_vpids
    ):
        ...
        for i in range(batch_size):
            ...
            for t in range(len(split_traj_embeds[i])):
                visited_vp_fts[gmap_vpids[i][1+t]] = torch.sum(i_traj_embeds[t], 0) / split_traj_vp_lens[i][t]
    ...
    def gmap_input_embedding(
        ...
        gmap_img_fts = self._aggregate_gmap_features(
            split_traj_embeds, split_traj_vp_lens, gmap_vpids
        )
