...
class R2RNavBatch(object):
    ...
    def eval_metrics_each_scan(self, preds):
        print('eval %d predictions' % (len(preds)))

        metrics = defaultdict(list)
        scan_metrics = defaultdict(dict)
        instruction_results = defaultdict(dict)
        for item in preds:
            instr_id = item['instr_id']
            traj = item['trajectory']
            scan, gt_traj = self.gt_trajs[instr_id]
            ...
            for k, v in traj_scores.items():
                metrics[k].append(v)
                scan_metrics[scan].setdefault(k, []).append(v)
            metrics['instr_id'].append(instr_id)
            instruction_results[scan][instr_id] = traj_scores['success']
        
        # with open("instruction_results_sample.json", "w") as f:
        #     json.dump(instruction_results, f, indent=2)
        ...
        scan_avg_metrics = {}
        for scan, scan_metric in scan_metrics.items():
            scan_avg_metrics[scan] = {
                'action_steps': np.mean(scan_metric['action_steps']),
                'steps': np.mean(scan_metric['trajectory_steps']),
                'lengths': np.mean(scan_metric['trajectory_lengths']),
                'nav_error': np.mean(scan_metric['nav_error']),
                'oracle_error': np.mean(scan_metric['oracle_error']),
                'sr': np.mean(scan_metric['success']) * 100,
                'oracle_sr': np.mean(scan_metric['oracle_success']) * 100,
                'spl': np.mean(scan_metric['spl']) * 100,
                'nDTW': np.mean(scan_metric['nDTW']) * 100,
                'SDTW': np.mean(scan_metric['SDTW']) * 100,
                'CLS': np.mean(scan_metric['CLS']) * 100,
            }

        return avg_metrics, scan_avg_metrics
